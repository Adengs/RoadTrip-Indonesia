...
public class AdapterMapsListLocation extends RecyclerView.Adapter<AdapterMapsListLocation.MyHolder> {
    public static final int REQ_REFRESH_PRODUCT_LIST = 2001;
    private Context context;
    private List<ResponseListLocation.DataEntity> item;
    private Fragment fragment;

    public AdapterMapsListLocation(Context context) {
                        ...
                        public void onResponse(Call<ResponseDetailExplore> call, Response<ResponseDetailExplore> response) {
                            if (response.body() != null) {
                                if (response.isSuccessful() && response.body().getSuccess()) {
                                    ...
                                    intent.putExtra("data-detail", new Gson().toJson(response.body().getData()));
                                    intent.putExtra("id-maps", response.body().getData().getId());
